...
acceptance: ## Run all acceptance tests
	...
	cd acceptance && SEALIGHTS_LOG_LEVEL=none go run -modfile "$$ACCEPTANCE_WORKDIR/tools/go.mod" gotest.tools/gotestsum --junitfile "$(ROOT_DIR)/acceptance.xml" -- -p 1 -timeout $(ACCEPTANCE_TIMEOUT) -tags=acceptance . -args -tags=@focus
